...
namespace Conectus.Members.IntergrationTests.Application.UseCases.Member.CreateMember
{
    ...
    public class CreateMemberTests
    {
        ...
        public async Task CreateAdultMemberSuccessfully()
        {
            ...
            member.FirstName.Should().Be(output.FirstName);
            member.LastName.Should().Be(output.LastName);
            member.DateOfBirth.Should().Be(output.DateOfBirth);
            member.Gender.Should().Be(output.Gender);
            member.PhoneNumber.Value.Should().Be(output.PhoneNumber);
            member.Document.Should().NotBeNull();
            member.Document.Document.Should().Be(output.Document.Number);
            member.Document.Type.Should().Be(output.Document.Type);
            member.Address.Should().NotBeNull();
            member.Address.Street.Should().Be(output.Address.Street);
            member.Address.Number.Should().Be(output.Address.Number);
            member.Address.Complement.Should().Be(output.Address.Complement);
            member.Address.District.Should().Be(output.Address.District);
            member.Address.City.Should().Be(output.Address.City);
            member.Address.State.Should().Be(output.Address.State);
            member.Address.ZipCode.Should().Be(output.Address.ZipCode);
            member.Address.Latitude.Should().Be(output.Address.Latitude);
            member.Address.Longitude.Should().Be(output.Address.Longitude);
